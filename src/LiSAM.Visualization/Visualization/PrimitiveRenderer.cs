...
public sealed class PrimitiveRenderer : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        GL.DeleteBuffer(_cuboidVbo);
        GL.DeleteVertexArray(_cuboidVao);
        _shader.Dispose();
        _disposed = true;
    }
    ...
    public void AddLines(ReadOnlySpan<CloudLine> lines)
    {
        lock (_sync)
        {
            ...
            foreach (CloudLine line in lines) _lines.Add(line);
    ...
    {
        foreach (TranslucentCuboid cuboid in cuboids) AddCuboid(cuboid);
    }
    ...
    private void UploadLinesIfChanged()
    {
        ...
        lock (_sync)
        {
            ...
            int offset = 0;
            foreach (CloudLine line in _lines)
            {
    ...
    private void UploadCuboidsIfChanged(Vector3 cameraPosition)
    {
        ...
        lock (_sync)
        {
            ...
            TranslucentCuboid[] sorted = _cuboids.OrderByDescending(c => (c.Center - cameraPosition).LengthSquared)
                .ToArray();
            data = new float[sorted.Length * CubeTriangleCorners.Length * FloatsPerVertex];
            int offset = 0;
            foreach (TranslucentCuboid cuboid in sorted)
            {
                Vector3[] corners = GetCorners(cuboid.Min, cuboid.Max);
                foreach (int corner in CubeTriangleCorners)
